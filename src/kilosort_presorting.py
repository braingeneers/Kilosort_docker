"""
Script to spike sort a list of recordings using Kilosort2 and SpikeInterface>=0.90
and convert results to .mat MATLAB file.
...
class RunKilosort:
    ...
    def setup_recording_files(self, recording, output_folder, verbose):
        ...
        # region Config text
        # Adapt to MATLAB standalone executable
        #      -- generate ops.mat and chanMap.mat matrices to replace config.m and chanMap.m
        # create the ops dictionary, save ops to config.mat
        # add comments to each line when this code passed test
        ops = {}
        ops_dict = {}
        ...
        savemat(str((output_folder / 'ops.mat').absolute()), ops)
    ...
    def execute_kilosort_file(output_folder, verbose):
        ...
        shell_cmd = f'''
                    ...
                    ./{stdln_script} {runtime_folder} {output_folder}
                '''
    ...
    def check_if_installed(self):
        if (Path(self.path) / 'kilosort_compiled').is_file():
            return True
        else:
            return False
...
class KilosortSortingExtractor:
    ...
    def __init__(self, folder_path, exclude_cluster_groups=None, keep_good_only=False):
        # region Import pandas and check if installed
        try:
            import pandas as pd
            have_pd = True
        except ImportError:
